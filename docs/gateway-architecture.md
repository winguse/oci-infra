...
- **Transports**: the `HTTPRoute` attaches to the existing catch-all `https` listeners of **both** the `envoy` (TCP: HTTP/1.1 + HTTP/2) and `envoy-quic` (UDP: HTTP/3) gateways, giving clients HTTP/1.1, HTTP/2, and HTTP/3 with the same DNS name + port 443. For HTTP/2 CONNECT tunneling, an `EnvoyPatchPolicy` `add`s `http2_protocol_options.allow_connect: true` (TCP gateway only), otherwise HTTP/2 proxy clients get a `400 http1.codec_error`. For HTTP/3, a second `EnvoyPatchPolicy` targets the **UDP data listener `envoy-quic/https-quic`** (NOT the TCP sibling `envoy-quic/https`) and `add`s `http3_protocol_options.allow_extended_connect: true`. HTTP/1.1 CONNECT works without either. NOTE: HTTP/1.1 and HTTP/2 both relay real tunneled bytes, but **HTTP/3 only authenticates to `200` and does not relay data** (known Envoy HTTP/3 CONNECT-stream data-relay limitation) — see `docs/repro-http-connect`.
- **Auth (no mTLS)**: a `SecurityPolicy.basicAuth` protects the route using the `proxy-auth` Secret (`.htpasswd` key, `{SHA}` format) in the `gw` namespace. Envoy Gateway's `SecurityPolicy.basicAuth` CRD only exposes `users` and would otherwise read the credential from the `Authorization` header; for a forward proxy the standard (RFC 7235) is the **`Proxy-Authorization`** header, so two `EnvoyPatchPolicy` resources (one per gateway) `add` `authentication_header: "Proxy-Authorization"` to the generated `envoy.filters.http.basic_auth/.../envoy-connect-proxy-auth` filter (HCM `http_filters/4`, `disabled: true` at listener level, enabled per-route via `BasicAuthPerRoute`). Envoy reads the credential header from that base filter config, so per-route enforcement honors `Proxy-Authorization` only — clients send `curl --proxy-header "Proxy-Authorization: Basic <b64>" -x https://<proxy-host>:443 ...`. The basic_auth filter always returns `401` on failure (no status override in its config); the same two `EnvoyPatchPolicy` resources add an HCM `local_reply_config` mapper (`status_code EQ 401 → 405`) so the proxy instead answers **405 Method Not Allowed**.
- **Chart wiring**:
